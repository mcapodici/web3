...
export function makeContractObject(web3: Web3, bankAccountAddress: string) {
  return new web3.eth.Contract(contractJson.abi as AbiItem[], bankAccountAddress);
}

export async function deposit(web3: Web3, bankAccountContractAddress: string,despoitEther: string,ownerAddress: string ) {
  const despoitWei = web3.utils.toWei(despoitEther, "ether");
  const contract = makeContractObject(web3, bankAccountContractAddress);
  await contract.methods.deposit().send({ from: ownerAddress, gas: 2000000, value: despoitWei });
}

export async function getExistingAccounts(web3: Web3, bankAccountFactoryContractAddress: string, ownerAddress: string) {
...
export async function createAccount(web3: Web3, bankAccountFactoryContractAddress: string, ownerAddress: string, initialDespoitEther: string) {
  const initialDespoitWei = web3.utils.toWei(initialDespoitEther, "ether");
  const factoryContract = makeFactoryContractObject(web3, bankAccountFactoryContractAddress);
